    ...
    public static function configure(Table $table): Table
    {
        return $table
            ->columns([
                ...
                TextColumn::make('processTask.taskable.name')
                    ->label('Entità')
                    ->badge()
                    ->sortable()
                    ->searchable()
                    ->formatStateUsing(fn($record) =>
                        $record->processTask->taskable_type === 'App\Models\PracticeScope'
                            ? $record->processTask->taskable->name
                            : ($record->processTask->taskable_type === 'App\Models\Company'
                                ? 'Azienda: ' . $record->processTask->taskable->name
                                : ($record->processTask->taskable_type === 'App\Models\Client'
                                    ? 'Cliente: ' . $record->processTask->taskable->name
                                    : ($record->processTask->taskable_type === 'App\Models\Project'
                                        ? 'Progetto: ' . $record->processTask->taskable->name
                                        : ($record->processTask->taskable_type === 'App\Models\Process'
                                            ? 'Processo: ' . $record->processTask->taskable->name
                                            : $record->processTask->taskable->name)))))),
