...
class CompanyWebsitesTable
{
    public static function configure(Table $table): Table
    {
        return $table
            ...
            ->headerActions([
                Action::make('scan_all_company_transparency')
                    ->label('Scansione Trasparenza Azienda')
                    ...
                    ->action(function () {
                        try {
                            ...
                            if (!$companyId) {
                                Notification::make()
                                    ->title('Errore')
                                    ->body('Nessuna azienda selezionata.')
                                    ->danger()
                                    ->send();
                                return;
                            }
            ...
            ->recordActions([
                EditAction::make(),
                Action::make('scan_transparency')
                    ->label('Scansione Trasparenza')
                    ->icon('heroicon-o-magnifying-glass')
                    ->color('warning')
                    ->requiresConfirmation()
                    ->modalHeading('Scansione Link Trasparenza')
                    ->modalDescription('Questo scansionerà il sito web alla ricerca di pagine di trasparenza ed estrarrà i documenti trovati. Continuare?')
                    ->modalSubmitActionLabel('Scansiona')
                    ->action(function (CompanyWebsite $record) {
                        try {
                            $scanService = app(TransparencyScanService::class);
                    ...
                    ->visible(function (CompanyWebsite $record) {
                        return !empty($record->transparency_date);
                    }),
            ])
            ->toolbarActions([
